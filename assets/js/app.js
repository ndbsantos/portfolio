(function(){
  ...
  async function loadProjects(){
    ...
    return data.map(p => ({
      ...
      slug: p.slug || slugify(p["ProjectClient"] ?? p["Project / Client"] ?? p.name || "")
  ...
  function projectCard(p){
    const tech = (p.technologies ? String(p.technologies).split(/,\s*/).slice(0,6) : []).filter(Boolean);
    return `
      <article class="card">
        <div class="project">
          <div>
        ...
        <p style="margin-top:14px"><a href="project.html?slug=${encodeURIComponent(p.slug)}">Ver detalhe →</a></p>
      </article>
    `;
  }
  ...
  function unique(arr){
    return Array.from(new Set(arr.filter(Boolean))).sort((a,b)=> a.localeCompare(b,'pt-PT'));
  }
  ...
  async function initProjectsPage(){
    ...
    const years = unique(projects.map(p => String(p.year))).sort((a,b)=> Number(b)-Number(a));
    ...
    function apply(){
      ...
      const filtered = projects.filter(p => {
        ...
        if(term){
          const blob = `${p.year} ${p.name} ${p.technologies||''} ${p.outcome||''} ${p.sector||''} ${p.role||''}`.toLowerCase();
          if(!blob.includes(term)) return false;
        }
        return true;
      });

      list.innerHTML = filtered.map(projectCard).join('') || `<div class="card"><p class="muted">Sem resultados.</p></div>`;
    ...
    clear.addEventListener('click', ()=>{ q.value=''; year.value=''; sector.value=''; role.value=''; apply(); });
  ...
  async function initProjectDetail(){
    ...
    const tech = (p.technologies ? String(p.technologies).split(/,\s*/) : []).filter(Boolean);
    techEl.innerHTML = tech.map(badge).join('') || '<span class="muted">(não especificado)</span>';
